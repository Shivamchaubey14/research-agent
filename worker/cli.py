"""Standalone CLI for the agent — run one research job without Kafka/Redis.

Useful for developing and debugging the agent loop in isolation; the production
path is the Kafka consumer in :mod:`worker.main`. Progress events print to
stderr and the final cited report to stdout:

    python -m worker.cli "What are the tradeoffs of Kafka vs RabbitMQ?" --depth deep
"""
import argparse
import json
import logging
